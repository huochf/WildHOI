...
from utils.optim_losses import MultiViewHOIKPSLoss, ContactLoss, ObjectScaleLoss, ObjectCoorLoss, SMPLDecayLoss, SMPLKPSLoss, CamDecayLoss
...
def load_occlusion_map(root_dir, object_name, image_ids):
    print('loading occlusion maps ...')
    smpl_occlusion_maps = []
    object_occlusion_maps = []
    for image_id in tqdm(image_ids):
        video_id, hoi_id, frame_id = image_id.split('_')
        occlusion_map_path = os.path.join(root_dir, object_name, 'occlusion_maps', video_id, hoi_id, '{}.pkl'.format(frame_id))
        occlusion_load = load_pickle(occlusion_map_path)

        smpl_occlusion_maps.append(occlusion_load['smpl_v_mask'])
        object_occlusion_maps.append(occlusion_load['object_v_mask'])

    smpl_occlusion_maps = np.stack(smpl_occlusion_maps, axis=0)
    object_occlusion_maps = np.stack(object_occlusion_maps, axis=0)

    return smpl_occlusion_maps, object_occlusion_maps
...
def optimize_all(args):
    ...
    results_dir = './outputs/optim_with_contact_kps_flow'
    ...
    # dataset = ObjectImageDataset(root_dir=os.path.join(args.root_dir, args.object, 'images_temp', video_id), frame_ids=frame_ids, bboxes=object_bboxes, out_res=224, coor_res=64)
    # dataloader = torch.utils.data.DataLoader(dataset, batch_size=8, num_workers=4, shuffle=False, drop_last=False)
    # coor_x2d, coor_x3d, coor_mask = get_object_coor(dataloader, object_model, object_name, smpl_princpt, smpl_bboxes, device)
    ...
    smpl_occlusion_maps, object_occlusion_maps = load_occlusion_map(root_dir, object_name, image_ids)
    mean_occlusion_map = load_pickle('./outputs/mean_occlusion/occlusion_map_mean_{}.pkl'.format(object_name))
    smpl_mean_occlusion_maps = mean_occlusion_map['smpl_occlusion_map_mean']
    object_mean_occlusion_maps = mean_occlusion_map['object_occlusion_map_mean']

    smpl_mean_occlusion_maps = torch.from_numpy(smpl_mean_occlusion_maps).float().to(device)
    object_mean_occlusion_maps = torch.from_numpy(object_mean_occlusion_maps).float().to(device)
    ...
    for begin_idx in range(0, n_seq, batch_size):
        ...
        _smpl_occlusion_maps = torch.from_numpy(smpl_occlusion_maps[begin_idx:begin_idx+batch_size]).float().to(device)
        _object_occlusion_maps = torch.from_numpy(object_occlusion_maps[begin_idx:begin_idx+batch_size]).float().to(device)


        loss_functions = [
            MultiViewHOIKPSLoss(model, _visual_feats, n_views=16).to(device),
            ContactLoss(_smpl_occlusion_maps, _object_occlusion_maps, smpl_mean_occlusion_maps, object_mean_occlusion_maps, threshold=0.05).to(device),
        ...
        for batch_idx, image_id in enumerate(_image_ids):
            hoi_recon_results.append({
                ...
                # 'smpl_v_contact': _smpl_occlusion_maps[batch_idx].detach().cpu().numpy().reshape(-1, ), # for debug
                # 'object_v_contact': _object_occlusion_maps[batch_idx].detach().cpu().numpy().reshape(-1),
            })
