...
def get_occlusion_maps(args):
    ...
    object_f = torch.cat([front_f, back_f + front_v.shape[0]], dim=0)
    ...
    for video_idx in range(begin_idx, end_idx):
        ...
        for hoi_instance in tracking_results['hoi_instances']:
            ...
            for item_idx, item in enumerate(tqdm(hoi_instance['sequences'][::fps])):
                ...
                if '_'.join([video_id, hoi_id, frame_id]) not in annotated_images:
                    continue
                ...
                rot_angle = object_pose_params[item_idx]['rot_angle']
                ...
                object_v = bicycle_v @ rotmat.transpose(-1, -2) + trans
