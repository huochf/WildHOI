...
class SmoothLoss(nn.Module):
    ...
    def forward(self, object_dict, batch_idx, batch_size):
        ...
        rot_angle = object_dict['rot_angle']

        loss_rot_angle_norm = (rot_angle ** 2).mean()
        if R6d.shape[0] > 1:
            ...
            loss_rot_angle = ((rot_angle[:-1] - rot_angle[1:]) ** 2).mean()
        else:
            loss_rot_angle = loss_r6d = loss_trans = torch.zeros(1).to(R6d.device)
        return {
            'loss_smooth_r6d': loss_r6d,
            'loss_smooth_trans': loss_trans,
            'loss_smooth_obj_v': loss_object_v,
            'loss_smooth_rot_angle': loss_rot_angle,
            'loss_rot_angle_norm': loss_rot_angle_norm,
        }
...
class Bicycle(nn.Module):
    ...
    def __init__(self, R6d_init, trans_init):
        super().__init__()
        ...
        rot_angle = torch.zeros(R6d_init.shape[0])
        self.rot_angle = nn.Parameter(rot_angle.float())
        self.R6d = nn.Parameter(R6d_init)
        self.trans = nn.Parameter(trans_init)


    def get_optimizer(self, lr=1e-3):
        optimizer = torch.optim.Adam([self.R6d, self.trans, self.rot_angle], lr=lr, betas=(0.9, 0.999))
        return optimizer


    def forward(self, batch_idx, batch_size):

        front_rotmat = axis_angle_to_matrix(self.rot_axis.reshape(1, 3) * self.rot_angle[batch_idx:batch_idx+batch_size].reshape(-1, 1))
        ...
        global_rotmat = rotation_6d_to_matrix(self.R6d[batch_idx:batch_idx+batch_size]) # [b, 3, 3]
        global_trans = self.trans[batch_idx:batch_idx+batch_size].unsqueeze(1) # [b, 1, 3]
        ...
        object_v = torch.cat([front_v, back_v], dim=1)

        return {
            'rotmat': global_rotmat,
            'R6d': self.R6d[batch_idx:batch_idx+batch_size],
            'trans': global_trans,
            'rot_angle': self.rot_angle[batch_idx:batch_idx+batch_size],
            'object_v': object_v,
        }
...
def extract_object_pose(args):
    ...
    for video_idx in range(args.begin_idx, args.end_idx):
        ...
        for hoi_instance in tracking_results['hoi_instances']:
            ...
            for x2d_np_, x3d_np_, mask_np_ in zip(coor_x2d, coor_x3d, binary_mask):
                try:
            ...
            trans_mean = np.mean(np.stack(trans), 0)
            rot_mean = np.mean(np.stack(rot_aa), 0)
            for idx, t in enumerate(trans):
                if np.abs(t - trans_mean).sum() > 10:
                    trans[idx] = trans_mean.reshape(-1)
            ...
            object_instance = Bicycle(R6d, T).to(device)
            ...
            loss_weights = {
                ...
                'loss_smooth_r6d': lambda cst, it: 10. ** 0 * cst / (1 + 10 * it),
                'loss_smooth_obj_v': lambda cst, it: 10. ** -2 * cst / (1 + 10 * it),
                'loss_corr': lambda cst, it: 10 ** 0 * cst / (1 + 10 * it),
                'loss_smooth_rot_angle': lambda cst, it: 10. ** 0 * cst / (1 + 10 * it),
                'loss_rot_angle_norm': lambda cst, it: 10. ** -2 * cst / (1 + 10 * it),
            }

            ################ do post optimization here ####################
            iterations = 3
            steps_per_iter_max = 500
            seq_len = all_corr_maps.shape[0]
            batch_size = 64
            for it in range(iterations):
                if it == 0:
                    object_instance.R6d.requires_grad_(False)
                    object_instance.rot_angle.requires_grad_(False)
                else:
                    object_instance.R6d.requires_grad_(True)
                    object_instance.rot_angle.requires_grad_(True)
                ...
                for i in loop:
                    ...
                    total_loss.backward()
                    torch.nn.utils.clip_grad_norm_([object_instance.R6d, object_instance.trans, object_instance.rot_angle], 0.1)
                    optimizer.step()
            ...
            rot_angle = object_outputs['rot_angle'].detach().cpu().numpy()
            ...
            for idx, frame_id in enumerate(image_ids):
                ...
                object_params['rot_angle'] = rot_angle[idx]
