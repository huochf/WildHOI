...
def render_coor_map():
    ...
    template = trimesh.load('../data/objects/{}.ply'.format(object_name), process=False)
    verts = torch.tensor(np.array(template.vertices), dtype=torch.float32)
    faces = torch.tensor(np.array(template.faces, dtype=np.int64))
    ...
    for file in tqdm(os.listdir(object_pose_dir)):
        ...
        _verts = verts.to(device).reshape(-1, 3)
        R = R.to(device).reshape(3, 3)
        T = T.to(device).reshape(1, 3)
        _verts = _verts @ R.transpose(0, 1) + T
        input_data = Meshes(verts=[_verts.cpu()], faces=[faces]).to(device)
